...
#rooms = [
#    {'id': 1, 'name':'python cource'},
#    {'id': 2, 'name':'css cource'},
#    {'id': 3, 'name':'game cource'}
#]
...
def loginPage(request):
    ...
    if request.method == 'POST':
        ...
        user = authenticate(request, email=email, password=password)# return back the user credentials

        if user is not None:
            login(request, user)
            return redirect('home')
        else:
            messages.error(request, 'Username or Password does not Found')
...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Error occurred while filling Registration Form')
...
def home(request):
    ...
    roomMessages = Message.objects.filter(Q(room__topic__name__icontains=q))[0:6]
...
def room(request, pk):
    ...
    if request.method == 'POST':
        roomMessages = Message.objects.create(
            user=request.user,
            room=room,
